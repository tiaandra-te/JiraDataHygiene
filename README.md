...
- `Jira.EnableJiraComments`: If `true`, add a Jira comment per issue using the filter description.
- `Jira.LogJiraCommentsToConsole`: If `true`, log comment text before posting.
...
- `SendGrid.SendReportOfLogs`: If `true`, send a run log email at the end of execution.
- `SendGrid.Email2SendReportOfLogs`: Recipient for the run log email.
